/**
 * Catalog of AutoRA components available in the editor palette. This module defines the
 * embedded set of theorists, experimentalists, and experiment runners (grouped by category)
 * that populate the workflow editor. Each component carries its protocol metadata and the
 * parameter definitions used to build and validate its configuration form.
 *
...
/**
 * Embedded component catalog, keyed by category (e.g. `experimentalists`, `theorists`,
 * `experiment-runners`). Each value is an array of component descriptors, where every
 * descriptor includes fields such as `uuid`, `protocolType`, `name`, `description`,
 * `githubCommit`, `pythonName`, `importPath`, `pipInstall`, `inputDataType`,
 * `outputDataType`, `file`, and `parameters` (a map of function name -> array of
